# -*- coding: utf-8 -*-
"""
Created on Tue Jul 23 16:07:12 2019
...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul 22 11:35:45 2019
...
Dinit = np.array([ -5.50833113e-07, 0])
...
observed1910 = np.copy(data[:, 18:23])
observed1910 [:, [0,1]] = observed1910 [:, [1,0]]

observed1900 = np.copy(data[:, 13:18])
observed1900 [:, [0,1]] = observed1900 [:, [1,0]]

observed1891 = np.copy(data[:, 8:13])
...
for year in observed.keys():
    observedTest[year] = observed[year][:, 0:3]
pop = {}
pop[1881] = data[:,2:5].sum(axis = 1)
pop[1891] = data[:,8:11].sum(axis = 1)
pop[1900] = data[:,13:16].sum(axis = 1)
pop[1910] = data[:,18:21].sum(axis = 1)

for year in observed.keys():
#    print(year)
    for i in range(len(speakerModelling.sum(axis = 1))):
        if (observed[year][:, 0:3].sum(axis = 1))[i]==0:
...
#            np.fill_diagonal(matrix, 1)
            for i in range(len(settlements0)):
                matrix[i,i]= math.exp(h[k]*pop[year-1][i])
            constant[languages[k]] = matrix
        score = []
...
for i in emptyplaces:
    speakerplot = np.insert(speakerplot, i, 0, axis = 0)
color1 = []
