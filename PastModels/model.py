...
Dinit = np.array([  2.08466626, 129.48974141,   1.58595933,   0.59353173,   0.74762004])
...
#                        matrix [i,j] =0
#                    else:
#                        matrix [i,j] = a/(4*math.pi*d)*math.exp(-1*distmat[i,j]/(4*d))
...
#                        matrix [i,j] = 1
        if d != 0:
            matrix = a/(4*math.pi*d)*np.exp(-1*distmat/(4*d))
        else:
            matrix = np.zeros((len(settlements),len(settlements)))
    ...
    return sse
#print(findsse(Dinit))
#res = opt.minimize(findsse, Dinit, method = 'Nelder-Mead', tol = 0.1)
#print(res.x)
#print(res.success)
#print(res.message)
#print(res.fun)
...
constant = {}
#d = abs(Dinit[-1])
#for k in range(len(Dinit)-1):
#    matrix = np.zeros((len(settlements),len(settlements)))
#    a = abs(Dinit[k])
for k in range(len(Dinit)):
    matrix = np.zeros((len(settlements),len(settlements)))
    d = abs(Dinit[k])
    for i in range(len(settlements)):
        for j in range(len(settlements)):
