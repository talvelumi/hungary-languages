...
Dinit = np.array([ 3.31074828e-01,  8.29843041e-01, -5.46174607e-07])
...
#            if d != 0:
#                if l == k:
#                    matrix = (1+factk)/(4*math.pi*d)*np.exp(-1*distmat/(4*d))
#                else:
#                    matrix = (1-factk)/(4*math.pi*d)*np.exp(-1*distmat/(4*d))
#            else:
...
#            dictl.append(matrix)
##        dictl = np.array(dictl)
#        constant[languages[k]] = dictl
...
#                        matrix = (1-factk)/(4*math.pi*d)*np.exp(-1*distmat/(4*d))
                    ...
                    matrix = np.zeros((len(settlements0),len(settlements0)))
                for i in range(len(settlements0)):
                    matrix[i,i]= math.exp((-1)**l*D[-1]*pop[year-1][i])
                dictl.append(matrix)
            constant[languages[k]] = dictl
        prob = []
        for j in range(langnum):
            ...
            for i in range(langnum):
                score.append((1+(1-2*(i^j))*factk)*constant[languages[j]][i] @ speaker[:,i])
#                score.append(constant[languages[j]][i] @ speaker[:,i])
            score = np.transpose(np.array(score))
            totalscore = score.sum(axis=1)
            prob.append(score[:,j] / totalscore)
        prob = np.transpose(np.array(prob))
        prob = prob / prob.sum(axis = 1)[:,None]
        speaker = prob * pop[year][:, None]
...
#                        sse += ((observedTest[year][i,j] - speaker[i,j])/observedTest[year][i,j])**2
    ...
    return sse
res = opt.minimize(findsse, np.append(Dinit, 0.01), method = 'Nelder-Mead', tol = 0.01)
print(res.x)
print(res.x)
print(res.success)
print(res.message)
